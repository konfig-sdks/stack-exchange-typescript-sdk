...
import { readableStreamToString, StackExchangeError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://stackexchange.com/oauth"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new StackExchangeError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new StackExchangeError(e, e.response?.data)
                }
                if (innerError instanceof StackExchangeError) {
                    // Got "StackExchangeError" from the above try block
                    throw innerError;
                }
